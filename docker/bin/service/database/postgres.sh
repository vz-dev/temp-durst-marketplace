...
function Database::init() {
    Compose::exec <<'EOF'
        ...
        psql -lqt \
            -h "${SPRYKER_DB_HOST}" \
            -U "${SPRYKER_DB_USERNAME}" |
            grep "${SPRYKER_DB_DATABASE}test" >/dev/null 2>&1 ||
            psql \
                -h "${SPRYKER_DB_HOST}" \
                -U "${SPRYKER_DB_ROOT_USERNAME}" \
                -tc "CREATE DATABASE \"${SPRYKER_DB_DATABASE}test\" OWNER = \"${SPRYKER_DB_USERNAME}\" ENCODING = 'UTF-8' LC_COLLATE='en_US.UTF-8' LC_CTYPE='en_US.UTF-8' CONNECTION LIMIT=-1 TEMPLATE=\"template0\";"

        psql \
            -h "${SPRYKER_DB_HOST}" \
            -U "${SPRYKER_DB_ROOT_USERNAME}" \
            -tc "GRANT ALL PRIVILEGES ON DATABASE \"${SPRYKER_DB_DATABASE}test\" TO \"${SPRYKER_DB_ROOT_USERNAME}\""
EOF
}
