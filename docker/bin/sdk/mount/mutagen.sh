...
function Mount::logs() {
    ...
    mutagen sync monitor "${SPRYKER_SYNC_MERCHANT_CENTER_SESSION_NAME}"
}
...
function Mount::Mutagen::beforeUp() {
    ...
    docker volume rm "${SPRYKER_SYNC_MERCHANT_CENTER_VOLUME}" >/dev/null 2>&1 || true
    ...
    sessionStatus=$(mutagen sync list "${SPRYKER_SYNC_MERCHANT_CENTER_SESSION_NAME}" 2>/dev/null | grep 'Status:' | awk '{print $2}' || echo '')
    if [ -z "${sessionStatus}" ] || [ "${sessionStatus}" == 'Halted' ]; then
        Console::verbose::start "${INFO}Cleaning previous synced files${NC}"
        mutagen sync terminate "${SPRYKER_SYNC_MERCHANT_CENTER_SESSION_NAME}" >/dev/null 2>&1 || true
        docker run -i --rm -v "${SPRYKER_SYNC_MERCHANT_CENTER_VOLUME}:/data" busybox find /data/ ! -path /data/ -delete >/dev/null 2>&1 || true
        Console::end "[OK]"
    fi
}
...
function Mount::Mutagen::beforeRun() {
    ...
    docker volume create --name="${SPRYKER_SYNC_MERCHANT_CENTER_VOLUME}" >/dev/null
    docker run -it --rm -v "${SPRYKER_SYNC_MERCHANT_CENTER_VOLUME}:/data" busybox chmod 777 /data >/dev/null 2>&1
    Console::end "[OK]"
}
...
function Mount::Mutagen::afterCliReady() {
    ...
    mutagen sync flush "${SPRYKER_SYNC_MERCHANT_CENTER_SESSION_NAME}"
}
