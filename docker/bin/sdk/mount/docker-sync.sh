...
function sync() {
    ...
    local syncMerchantCenterVolume="${SPRYKER_DOCKER_PREFIX}_merchant_center_${SPRYKER_DOCKER_TAG}_data_sync"

    case "${command}" in
        create)
            Console::verbose "${INFO}Creating 'data-sync' volume${NC}"
            docker volume create --name="${syncVolume}" >/dev/null
            Console::verbose "${INFO}Creating Merchant Center 'data-sync' volume${NC}"
            docker volume create --name="${syncMerchantCenterVolume}" >/dev/null
            ;;
        ...
        clean)
            ...
            docker volume rm "${syncMerchantCenterVolume}" >/dev/null 2>&1 || true
            ;;
